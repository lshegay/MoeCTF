...
const NeDBStore = (options: DatastoreOptions): Store => {
  const backend = backendNeDB(session);
...
function backendNeDB (connect) {
   ...
   */
  function NeDBStore(this: any, options): void {
    var onLoadFn, aci;
    const _this = this;
    ...
    _this.datastore = Datastore.create(options);
  ...
  NeDBStore.prototype.touch = function (sessionId, session, callback) {
    var touchSetOp: any = { updatedAt: new Date() };
