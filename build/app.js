        ...
        db.users.findOne({ name: config.adminCreditals.username }, (error, user) => {
            ...
            if (!user) {
                db.users.insert({
                    ...
                    password: crypto_1.default.pbkdf2Sync(config.adminCreditals.password, config.secret, 1, 32, 'sha512').toString('hex'),
